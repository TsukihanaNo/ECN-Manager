...
class UserPanel(QtWidgets.QWidget):
    ...
    def loadInfo(self):
        ...
        result = self.cursor.fetchone()
        check_dict = {"y":QtCore.Qt.Checked,"n":QtCore.Qt.Unchecked}
    ...
    def savePermissions(self):
        ...
        if result is not None:
            data = (check_dict[self.tab_permissions.check_ecn.checkState()],check_dict[self.tab_permissions.check_pcn.checkState()],check_dict[self.tab_permissions.check_create_user.checkState()],check_dict[self.tab_permissions.check_reject_signer.checkState()],check_dict[self.tab_permissions.check_settings.checkState()],check_dict[self.tab_permissions.check_view_analytics.checkState()],self.tab_general.line_user.text())
            self.cursor.execute(f"UPDATE PERMISSIONS SET CREATE_ECN = ?, CREATE_PCN = ?, CREATE_USER = ?, REJECT_SIGNER = ?, ACCESS_SETTINGS = ?, VIEW_ANALYTICS = ? WHERE USER_ID = ?", (data))
        else:
            data = (self.tab_general.line_user.text(),check_dict[self.tab_permissions.check_ecn.checkState()],check_dict[self.tab_permissions.check_pcn.checkState()],check_dict[self.tab_permissions.check_create_user.checkState()],check_dict[self.tab_permissions.check_reject_signer.checkState()],check_dict[self.tab_permissions.check_settings.checkState()],check_dict[self.tab_permissions.check_view_analytics.checkState()])
            self.cursor.execute(f"INSERT INTO PERMISSIONS(USER_ID,CREATE_ECN, CREATE_PCN, CREATE_USER, REJECT_SIGNER, ACCESS_SETTINGS, VIEW_ANALYTICS) VALUES (?,?,?,?,?,?,?)",(data))
        self.db.commit()
